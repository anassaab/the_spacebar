    ...
    create(context) {
        function canBeProperty(prop, name) {
            return (prop.type === 'SpreadElement' ||
                utils_1.isIdentifier(prop.key, name) ||
                (nodes_1.isLiteral(prop.key) && prop.key.value === name));
        }
        function isCallToAdmZipExtractAll({ callee }) {
            return callee.type === 'MemberExpression' && utils_1.isIdentifier(callee.property, 'extractAllTo');
        }
        function isAdmZipLibraryInScope() {
            return isAdmZipLibraryImported() || isAdmZipLibraryRequired();
        }
        function isAdmZipLibraryImported() {
            return utils_1.getImportDeclarations(context).some(i => i.source.value === ADM_ZIP);
        }
        function isAdmZipLibraryRequired() {
            return utils_1.getRequireCalls(context).some(r => r.arguments[0].type === 'Literal' && r.arguments[0].value === ADM_ZIP);
        }
        function isSensiteTarCall(call) {
            if (utils_1.isCallToFQN(context, call, 'tar', 'x')) {
        ...
        function isSensiteExtractZipCall(call) {
            ...
            if (call.callee.type === 'Identifier' &&
                ((_a = utils_1.getModuleNameOfIdentifier(context, call.callee)) === null || _a === void 0 ? void 0 : _a.value) === 'extract-zip') {
                const secondArg = call.arguments.length > 1 ? call.arguments[1] : null;
                if (!secondArg) {
                    return false;
                }
                const secondArgValue = utils_1.getValueOfExpression(context, secondArg, 'ObjectExpression');
        ...
        return {
            CallExpression(node) {
                ...
                if (isSensiteTarCall(call) ||
                    ...
                    utils_1.isCallToFQN(context, call, 'jszip', 'loadAsync') ||
                    utils_1.isCallToFQN(context, call, 'yauzl', 'open') ||
