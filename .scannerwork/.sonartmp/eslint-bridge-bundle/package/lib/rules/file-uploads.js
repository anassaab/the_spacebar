...
function checkCallExpression(context, callExpression) {
    ...
    const moduleName = utils_1.getModuleNameOfImportedIdentifier(context, callee) ||
        utils_1.getModuleNameOfIdentifier(context, callee);
...
function checkFormidable(context, callExpression) {
    ...
    if (callExpression.arguments.length === 0) {
        const formVariable = utils_1.getLhsVariable(context);
        if (formVariable) {
    ...
    const options = utils_1.getValueOfExpression(context, callExpression.arguments[0], 'ObjectExpression');
    if (options) {
        report(context, !!utils_1.getObjectExpressionProperty(options, UPLOAD_DIR), keepExtensionsValue((_a = utils_1.getObjectExpressionProperty(options, KEEP_EXTENSIONS)) === null || _a === void 0 ? void 0 : _a.value), callExpression);
    }
}
function checkMulter(context, callExpression) {
    ...
    const multerOptions = utils_1.getValueOfExpression(context, callExpression.arguments[0], 'ObjectExpression');
    if (!multerOptions) {
        return;
    }
    const storagePropertyValue = (_a = utils_1.getObjectExpressionProperty(multerOptions, STORAGE_OPTION)) === null || _a === void 0 ? void 0 : _a.value;
    if (storagePropertyValue) {
        const storageValue = utils_1.getValueOfExpression(context, storagePropertyValue, 'CallExpression');
        if (storageValue) {
...
function getDiskStorageCalleeIfUnsafeStorage(context, storageCreation) {
    ...
    if (args.length > 0 && isMemberWithProperty(callee, 'diskStorage')) {
        const storageOptions = utils_1.getValueOfExpression(context, args[0], 'ObjectExpression');
        if (storageOptions && !utils_1.getObjectExpressionProperty(storageOptions, DESTINATION_OPTION)) {
            return callee;
        }
    }
    return false;
...
function getVariablePropertyFromAssignment(context, assignment) {
    ...
    if (memberExpr.object.type === 'Identifier' && memberExpr.property.type === 'Identifier') {
        const objectVariable = utils_1.getVariableFromName(context, memberExpr.object.name);
        if (objectVariable) {
...
function report(context, uploadDirSet, keepExtensions, callExpression, secondaryLocation) {
    ...
    if (message) {
        if (secondaryLocation) {
            message = utils_1.toEncodedMessage(message, [secondaryLocation.node], [secondaryLocation.message]);
        }
        else {
            message = utils_1.toEncodedMessage(message, []);
        }
        context.report({
            message,
