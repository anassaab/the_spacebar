...
function decorateTypescriptEslint(rule) {
    return decorators_1.interceptReport(rule, reportExempting(expr => isNegatedIife(expr) ||
...
function decorateJavascriptEslint(rule) {
    return decorators_1.interceptReport(rule, reportExempting(expr => isNegatedIife(expr) || isSequenceWithSideEffects(expr)));
}
