...
exports.rule = {
    create(context) {
        function isChmodLikeFunction(node) {
            ...
            return utils_1.isIdentifier(callee.property, ...chmodLikeFunction);
        }
        ...
        function modeFromMemberExpression(modeExpr) {
            ...
            if (utils_1.isMemberExpression(object, 'fs', 'constants') && property.type === 'Identifier') {
        ...
        function modeFromExpression(expr, visited) {
            ...
            if (expr.type === 'MemberExpression') {
                ...
                const usage = utils_1.getUniqueWriteUsage(context, expr.name);
        ...
        return {
            CallExpression: (node) => {
                ...
                else if (utils_1.isCallToFQN(context, callExpression, 'process', 'umask')) {
