        ...
        return {
            NewExpression: (node) => {
                ...
                if (newExpression.arguments.length === 0) {
                    context.report({ node: callee, message: utils_1.toEncodedMessage(MESSAGE, []) });
                    return;
                }
                ...
                if (secrets &&
                    ...
                    context.report({ node: callee, message: utils_1.toEncodedMessage(MESSAGE, [secrets]) });
                }
                else if (!secrets) {
                    context.report({ node: callee, message: utils_1.toEncodedMessage(MESSAGE, [firstArgument]) });
                }
            },
        };
    },
