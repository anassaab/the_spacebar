...
exports.rule = utils_express_1.Express.SensitiveMiddlewarePropertyRule(findDirectivesWithSensitiveFrameAncestorsPropertyFromHelmet, `Make sure disabling content security policy frame-ancestors directive is safe here.`);
...
function isValidHelmetModuleCall(context, callExpr) {
    ...
    if (callee.type === 'MemberExpression' &&
        ((_b = utils_1.getModuleNameOfNode(context, callee.object)) === null || _b === void 0 ? void 0 : _b.value) === HELMET &&
...
function getFrameAncestorsProperty(directives) {
    ...
    for (const propertyKey of propertyKeys) {
        const maybeProperty = utils_1.getObjectExpressionProperty(directives.value, propertyKey);
        if (maybeProperty) {
