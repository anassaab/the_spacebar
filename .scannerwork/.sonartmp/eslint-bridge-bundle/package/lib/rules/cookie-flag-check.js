...
class CookieFlagCheck {
    ...
    checkCookiesMethodCall(callExpression) {
        if (!utils_1.isIdentifier(callExpression.callee.property, 'set')) {
            return;
        }
    ...
    checkCsurf(callExpression) {
        ...
        if (cookieProperty) {
            ...
            const cookiePropertyLiteral = utils_1.getValueOfExpression(this.context, cookieProperty.value, 'Literal');
            if ((cookiePropertyLiteral === null || cookiePropertyLiteral === void 0 ? void 0 : cookiePropertyLiteral.value) === true) {
                this.context.report({
                    node: callExpression.callee,
                    message: utils_1.toEncodedMessage(this.issueMessage, [cookiePropertyLiteral]),
                });
            }
        }
    }
    ...
    checkSensitiveCookieArgument(callExpression, sensitiveArgumentIndex) {
        ...
        const cookieObjectExpression = utils_1.getValueOfExpression(this.context, sensitiveArgument, 'ObjectExpression');
    ...
    checkSensitiveObjectArgument(callExpression, argumentIndex) {
        ...
        const objectExpression = utils_1.getValueOfExpression(this.context, firstArgument, 'ObjectExpression');
        if (!objectExpression) {
            return;
        }
        const cookieProperty = utils_1.getObjectExpressionProperty(objectExpression, 'cookie');
        if (!cookieProperty) {
            return;
        }
        const cookiePropertyValue = utils_1.getValueOfExpression(this.context, cookieProperty.value, 'ObjectExpression');
    ...
    checkFlagOnCookieExpression(cookiePropertyValue, firstArgument, objectExpression, callExpression) {
        const flagProperty = utils_1.getObjectExpressionProperty(cookiePropertyValue, this.flag);
        if (flagProperty) {
            const flagPropertyValue = utils_1.getValueOfExpression(this.context, flagProperty.value, 'Literal');
            if ((flagPropertyValue === null || flagPropertyValue === void 0 ? void 0 : flagPropertyValue.value) === false) {
                ...
                this.context.report({
                    node: callExpression.callee,
                    message: utils_1.toEncodedMessage(this.issueMessage, secondaryLocations),
                });
            }
        }
    }
    checkCookiesFromCallExpression(node) {
        ...
        const moduleName = utils_1.getModuleNameOfNode(this.context, callee);
        ...
        if (callee.type === 'MemberExpression') {
            const objectValue = utils_1.getValueOfExpression(this.context, callee.object, 'NewExpression');
            if (objectValue) {
                const module = utils_1.getModuleNameOfNode(this.context, objectValue.callee);
