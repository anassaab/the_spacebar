    ...
    create(context) {
        ...
        return {
            BinaryExpression: (node) => {
                ...
                const leftType = utils_1.getTypeFromTreeNode(binaryExpression.left, services);
                const rightType = utils_1.getTypeFromTreeNode(binaryExpression.right, services);
                if (operator === '+') {
            ...
            AssignmentExpression: (node) => {
                ...
                const leftType = utils_1.getTypeFromTreeNode(assignmentExpression.left, services);
                const rightType = utils_1.getTypeFromTreeNode(assignmentExpression.right, services);
                if (operator === '+=') {
            ...
            'UnaryExpression[operator="-"]': (node) => {
                ...
                const type = utils_1.getTypeFromTreeNode(unaryExpression.argument, services);
            ...
            UpdateExpression: (node) => {
                ...
                const type = utils_1.getTypeFromTreeNode(updateExpression.argument, services);
                if (isBooleanStringOrDate(type)) {
                    context.report({
...
function checkPlus(leftType, rightType, expression, context) {
    if (isNumber(leftType) && isBooleanOrDate(rightType)) {
        context.report({
            ...
            message: utils_1.toEncodedMessage(MESSAGE, [expression.left]),
        });
    }
    if (isNumber(rightType) && isBooleanOrDate(leftType)) {
        context.report({
            node: expression.left,
            message: utils_1.toEncodedMessage(MESSAGE, [expression.right]),
        });
    }
}
...
function checkArithmetic(leftType, rightType, expression, context) {
    ...
    if (secondaryLocations.length !== 0) {
        context.report({
            node: expression,
            message: utils_1.toEncodedMessage('Convert the operands of this operation into numbers.', secondaryLocations),
        });
    }
}
...
function isBooleanStringOrDate(type) {
    var _a;
    return isBoolean(type) || utils_1.isStringType(type) || ((_a = type.symbol) === null || _a === void 0 ? void 0 : _a.name) === 'Date';
}
