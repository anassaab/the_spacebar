...
const BODY_PARSER_DEFAULT_SIZE = bytes_1.parse('100kb');
...
function checkCallExpression(context, callExpression) {
    ...
    const moduleName = utils_1.getModuleNameOfImportedIdentifier(context, identifierFromModule) ||
        utils_1.getModuleNameOfIdentifier(context, identifierFromModule);
    if (!moduleName) {
        return;
    }
...
function checkFormidable(context, callExpression) {
    ...
    if (options) {
        const property = utils_1.getObjectExpressionProperty(options, MAX_FILE_SIZE);
...
function checkMulter(context, callExpression) {
    ...
    const limitsPropertyValue = (_a = utils_1.getObjectExpressionProperty(multerOptions, LIMITS_OPTION)) === null || _a === void 0 ? void 0 : _a.value;
    if (limitsPropertyValue && limitsPropertyValue.type === 'ObjectExpression') {
        const fileSizeProperty = utils_1.getObjectExpressionProperty(limitsPropertyValue, FILE_SIZE_OPTION);
...
function checkBodyParser(context, callExpression) {
    ...
    const limitsProperty = utils_1.getObjectExpressionProperty(options, LIMITS_OPTION);
...
function visitAssignment(context, assignment) {
    const variableProperty = file_uploads_1.getVariablePropertyFromAssignment(context, assignment);
    if (!variableProperty) {
        return;
    }
...
function getSizeValue(context, node) {
    const literal = utils_1.getValueOfExpression(context, node, 'Literal');
    if (literal) {
        if (typeof literal.value === 'number') {
            return literal.value;
        }
        else if (typeof literal.value === 'string') {
            return bytes_1.parse(literal.value);
        }
    }
    return undefined;
}
