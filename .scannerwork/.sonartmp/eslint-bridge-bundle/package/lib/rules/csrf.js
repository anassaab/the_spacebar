        ...
        function checkIgnoredMethods(node) {
            if (node.value.type === 'ArrayExpression') {
                ...
                if (unsafeMethods.length > 0) {
                    ...
                    context.report({
                        message: utils_1.toEncodedMessage('Make sure disabling CSRF protection is safe here.', rest),
                        node: first,
                    });
                }
            }
        ...
        function checkCallExpression(callExpression) {
            ...
            if (callee.type === 'MemberExpression') {
                ...
                if (utils_1.isIdentifier(callee.property, 'post', 'put', 'delete', 'patch') &&
                    ...
                    context.report({
                        message: utils_1.toEncodedMessage('Make sure not using CSRF protection is safe here.', []),
                        node: callee,
                    });
                }
            }
